...
**Prove**: Set $\delta_{gf} = min(\delta_{f}, \delta_{g})$, if $|x - a| < \delta_{gf} \Rightarrow |f(x) - f(a)| < \epsilon, \text{~set~} y = f(x) \Rightarrow |y - f(a)| < \epsilon \le \delta_{g} \Rightarrow |g(y) - g(f(a))| < \epsilon \Rightarrow |g(f(x)) - g(f(a))| < \epsilon$. Thus $g \circ f$ at $a$.  