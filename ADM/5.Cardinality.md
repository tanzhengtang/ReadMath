...
**Prove:** Suppose $\mathscr{A} = \{A_1,....,A_n...\}$ is denumerable and all $A_n \in \mathscr{A}$ are disjoint denumerable, for every $A_n = \{a^1_n,.....a^m_n....\}$, then $\bigcup\limits_{A \in \mathscr{A}} A = \{a^1_1,.....a^m_1....a^n_n...\}$. Let function $f: N * N \rightarrow \bigcup\limits_{A \in \mathscr{A}} A$, and $f((n, m)) = a^m_n$, it's easy to prove $f$ is one-to-one correspondence function. Then Use Theorem 5.2.3 we have $\bigcup\limits_{A \in \mathscr{A}} A$ is also countable. The rest is skip.  
...
### Lemma 5.5.2  
Let $\{A_i: i \in N\}$ be a denumerable family of sets. For each $i \in N$, let $B_i = A_i - $