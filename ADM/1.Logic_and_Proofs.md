...
**Answer:** $[(\forall x)P(x) \rightarrow (\forall x)Q(x)]$ must be true and $(\forall x)[P(x) \rightarrow Q(x)]$ must be false. $(\forall x)P(x)$ is false $\rightarrow [(\forall x)P(x) \rightarrow (\forall x)Q(x)]$ is true. Now we could set $P(x)$ is $x > 0$ in $R$. If we want $(\forall x)[P(x) \rightarrow Q(x)]$ false then $(\exist x)[P(x)  \wedge \sim Q(x)]$ true. Set $\sim Q(x)$ be $x > 1$, thus $Q(x) = x \le 1$. Consider $(\forall x)[x > 0 \rightarrow x \le 1]
...
**Prove:** Obviously, $d/c$ divdes $a/c$ and $b/c$, so $d/c \le gcd(a/c,b/c)$. According to condition if question, $d = sa + tb \rightarrow d/c = sa/c + tb/c$, then $d/c$ is a linear combination of $a/c$ and $b/c$, thus $gcd(a/c,b/c)$ divides $d/c \rightarrow d/c \ge gcd(a/c,b/c) \rightarrow d/c = gcd(a/c,b/c)$.  
