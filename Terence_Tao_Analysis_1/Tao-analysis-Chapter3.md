...
* Let $C = X * Y$, use Lemma 3.4.9, $H = \{M: M \subset C\}$. And use specification axiom to select all sets which statisfy the condition of $G$ in exercise 3.5.10 to construct a set $P$. Then $\forall G \in P$ exist a function $f$ such that whose graph is equal to $G$ to construct a set $F$ by replacement axiom. Suppose a arbitrary function $f^\prime: X \rightarrow Y$, then that could have a set $G^\prime = \{(x, f(x)): x \in X, f(x) \in Y\}$, obviously $G^\prime \subset C$ and $G \in P$, so the $F$ is the set $Y^X$.  
...
* If $X$ is empty, it's impossible for that there exists a bijection function from empty set to non-empty set. Because only function from $\empty$ to $\empty$ could be surjective.     
...
(b) Let $X$ and $Y$ be finite sets. Then $X \cup Y$ is finite and $\#(X \cup Y) \le \#(X) + \#(Y)$. If in addition $X$ and $Y$ are disjoint (i.e., $X \cap Y = \empty$), then $\#(X \cup Y) = \#(X) + \#(Y)$.  
...
* If $f$ is not one-to-one, then at least have two different elements $x, y \in X$ such that $f(x) = f(y)$. Suppose $X^\prime = X - \{x\}$ and $g: X^\prime \rightarrow Y, g(x) = f(x)$, then $f(X) = g(X^\prime)$. Because $g$ is a bijection, $\#(f(X)) = \#(g(X^\prime)) = \#(X^\prime) < \#(X)$.  

(e) Let $X$ and $Y$ be finite sets. Then Cartesian product $X * Y$ is finite and $\#(X * Y) = \#(X) * \#(Y)$.  
* $X * Y = \{(x,y):x\in X \ and \ y\in Y\} = \bigcup \{(x,y_i):x \in X, 1 \le i \le \#(Y)\}$, let $Y_i = ((x,y_i):x \in X, 1 \le i \le \#(Y))$, then $\#(X * Y) = \#(Y_1 \cup Y_2 .... \cup Y_{\#(Y)})$. Obviously $Y_1......Y_{\#(Y)}$ is disjoint with each other, so $\#(X * Y) = \#(Y_1 \cup Y_2 .... \cup Y_{\#(Y)}) = \#(Y_1) + \#(Y_2) + ......+ \#(Y_{\#(Y)})$. It's easy to find a bijection from $X$ to $Y_i$, then $\#(Y_i) = \#(X)$. So that we get $\#(X * Y) = \#(Y_1) + \#(Y_2) + ......+ Y_{\#(Y)} = \#(X) * \#(Y)$.  

(f) Let $X$ and $Y$ be finite sets. Then the set $Y^X$ (defined in Axiom 3.10) is finite and $\#(Y^X) = \#(Y)^{\#(X)}$.  
* Suppose $X$ has $n$ cardinality and $Y$ has $m$ cardinality, thus $X = \{x_1, ...., x_n\}, Y = \{y_1, ...., y_m\}$ and $X * Y = \{(x, y): x \in X, y \in Y\}$, use the axiom of specification $X_{i(1 \le i \le n)} = \{(x_i, y): y \in Y\}$ and $\#(X_i) = \#(Y) = m$. By Exercise 3.5.10, the graph of function is equal to function itself, then for one specific function(denote $f$) graph is $\{(x_1, f(x_1)), ...., (x_n, f(x_n))\}$. The graph also has $n$ cardinlity and its every element from $X_{i(1 \le i \le n)}$, so the all function graph could be $\prod_1^n X_i = \{(g_i): g_i \in X_i, 1 \le i \le n\}$. One of element in $\prod_1^n X_i$ repsent a function graph with domain $X$ and range $Y$. Thus we have $\#(\prod_{i = 1}^n X_i)= \#(Y^X)$.  
...
Exercise 3.6.3. Let $n$ be a natural number, and let $f: \{i \in N : 1 \le i \le n\} \rightarrow N$ be a function. Show that there exists a natural number $M$ such that $f(i) \le M$ for all $1 \le i \le n$. (Hint: induct on $n$. You may also want to peek at Lemma 5.1.14.) Thus finite subsets of the natural numbers are bounded.  
...
* Suppose have a finite subset of $N$, we could say this set $A$ has $m$ cardinality, which means $A$ has a bijection $g$ to $B = \{i \in N:1 \le i \le n\}$ for some natural number $n$. And $g^{-1}$ is from $B$ to $A$, obviously $g^{-1}(B) = A \subseteq N$, now we could say $g^{-1}(i) \le M \ \text{for} \ 1 \le i \le n$ which means $A$ is bounded.  
...
Exercise 3.6.10. Let $A_1,..., A_n$ be finite sets such that $\#(\bigcup_{i \in \{1,...,n\}}A_i) > n$. Show that there exists $i \in \{1, ..., n\}$ such that $\#(A_i) \ge 2$. (This is known as the pigeonhole principle.)  
* 3.6.10 Use (b) of proposition 3.6.14, then $\#(\bigcup_{i \in (1,....,n)}A_i) = \#(A_1 \bigcup A_2.......\bigcup A_n) \le \#(A_1)+.....+ \#(A_n)$, and $\#(\bigcup_{i \in (1,....,n)}A_i) > n$, so $\#(A_1)+.....+ \#(A_n) > n$, If there does not exist $i \in \{1, ..., n\}$ such that $\#(A_i) ≥ 2$, then every $\#(A_i)\le 1$, obviously $\#(A_1)+.....+ \#(A_n) \le n$, it's a contradiction.  