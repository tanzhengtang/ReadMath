...
* Suppse $n = 0$, let $A_n = \lbrace 0 \rbrace, B_n = \lbrace 1, 2, .... \rbrace$. Obviously, the $A_n, B_n$ obey the five properties. Suppose when $n = k$ that also get unique pair $A_k, B_k$. Consider the status of $A_n, B_n$ when $n = k++$.  
* Use sepcification and union axim, $A_{k++} = \lbrace A_k \rbrace \cup \lbrace k++ \rbrace, B_{k++} = B_k / \lbrace k++ \rbrace$
