...
* Suppose $g \circ f = g^\prime \circ f$. $\forall y \in Y, \exists x \in X \  st.\  f(x) = y$. Because $g(f(x)) = g^\prime(f(x)) \Rightarrow g(y) = g^\prime(y)$ for every $y$ in $Y$. Thus $g = g^\prime$. If $f$ is not surjective, then there exist some $y^\prime \in Y \ st. \exists x^\prime \notin X, f(x^\prime) = y^\prime$, it's possible that $g(y^\prime) \ne g^\prime(y^\prime)$.  
