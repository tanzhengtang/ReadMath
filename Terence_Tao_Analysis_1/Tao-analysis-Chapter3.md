...
* 3.5.7 (Ordered $n$-tuple and $n$-fold Cartesian product). Let $n$ be a natural number. An ordered $n$-tuple$\ (x_i)_{1 \leq i \leq n}$ (also denoted $(x_1, ..., x_n)$) is a collection of objects $x_i$, one for every natural number $i$ between $i$ and $n$; we refer to $x_i$ as the ith component of the ordered $n$-tuple. Two ordered $n$-tuples $(x_i)_{1 \leq i \leq n}$ and $(y_i)_{1 \leq i \leq n}$ are said to be equal iff $x_i = y_i$ for all $1 \leq i \leq n$. If $(X_i)_{1 \leq i \leq n}$ is an ordered $n$-tuple of sets, we define their Cartesian product $\prod_{1 \leq i \leq n} X_i$ (also denoted $\prod_{i = 1}^n X_i \text{\ or\ } X_1 * . . . * X_n$) by $\prod_{1 \le i \le n} X_i := \lbrace (x_i)_{1 \le i \le n} : x_i \in X_i \text{\ for\ all\ } 1 \le i \le n \rbrace$.  
